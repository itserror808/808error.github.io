...
// Get the total number of 'house' kits
$query = $pdo->prepare("SELECT COUNT(*) as total FROM kits WHERE category = 'house'");
$query->execute();
...
// Fetch 'house' kits data for the current page from the database
try {
    $query = $pdo->prepare("SELECT * FROM kits WHERE category = 'house' LIMIT :startIndex, :itemsPerPage");
...
<div>
    ...
    <div class="kits-grid">
        <?php
        // Generate HTML for each house kit item
        foreach ($kits as $kit) {
            $name = $kit['name'];
            $price = $kit['price'];
            $link = $kit['link'];

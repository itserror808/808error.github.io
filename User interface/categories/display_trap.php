...
// Get the total number of 'trap' kits
$query = $pdo->prepare("SELECT COUNT(*) as total FROM kits WHERE category = 'trap'");
$query->execute();
...
// Fetch 'trap' kits data for the current page from the database
try {
    $query = $pdo->prepare("SELECT * FROM kits WHERE category = 'trap' LIMIT :startIndex, :itemsPerPage");
...
<div>
    ...
    <div class="kits-grid">
        <?php
        // Generate HTML for each trap kit item
        foreach ($kits as $kit) {
            $name = $kit['name'];
            $price = $kit['price'];
            $link = $kit['link'];

...
// Get the total number of 'drill' kits
$query = $pdo->prepare("SELECT COUNT(*) as total FROM kits WHERE category = 'drill'");
$query->execute();
...
// Fetch 'drill' kits data for the current page from the database
try {
    $query = $pdo->prepare("SELECT * FROM kits WHERE category = 'drill' LIMIT :startIndex, :itemsPerPage");
...
<div>
    ...
    <div class="kits-grid">
        <?php
        // Generate HTML for each drill kit item
        foreach ($kits as $kit) {
            $name = $kit['name'];
            $price = $kit['price'];
            $link = $kit['link'];
